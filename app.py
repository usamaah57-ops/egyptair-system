...
if 'active_id' in st.session_state:
    ...
    if data:
        ...
        # استرجاع التاريخ أو وضع تاريخ اليوم كبديل
        f_date = data.get('date', datetime.now(BAGHDAD_TZ).strftime('%d-%m-%Y'))
        st.info(f"📅 تاريخ الرحلة: {f_date}")
        ...
        if st.button("📧 إرسال التقرير النهائي", type="primary", use_container_width=True):
            try:
                ...
                # طباعة بيانات الرحلة في الـ PDF (تأكيد طباعة التاريخ)
                pdf.set_font("Arial", 'B', 12)
                pdf.cell(63, 10, f"Flight: {data.get('flight_no')}", 1, 0, 'C')
                pdf.cell(63, 10, f"Reg: {data.get('reg')}", 1, 0, 'C')
                pdf.cell(64, 10, f"Date: {f_date}", 1, 1, 'C') # هنا يتم طباعة التاريخ
                pdf.ln(10)
                ...
                msg['Subject'] = f"Ops Report: {data.get('flight_no')} | {data.get('reg')} | {f_date}"
                ...
                os.remove(os.path.join(DATA_DIR, f"{st.session_state.active_id}.json"))
                st.success("تم الإرسال والأرشفة!")
                st.balloons()
                st.rerun()
