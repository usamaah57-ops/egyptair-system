...
# --- 4. وظائف الحفظ التلقائي (Auto-Save Logic) ---
def save_flight_data(flight_id, data):
    """حفظ البيانات فورياً في ملف JSON"""
...
option = st.sidebar.radio("القائمة:", ["الرحلات النشطة", "فتح رحلة جديدة"])

if option == "فتح رحلة جديدة":
    st.subheader("🆕 تسجيل رحلة جديدة")
    new_f = st.text_input("رقم الرحلة:").upper().strip()
    new_r = st.text_input("تسجيل الطائرة:").upper().strip()
    ...
    if st.button("بدء الرحلة"):
        if new_f and new_r:
            ...
            if f_id in active_flights:
                st.error("الرحلة موجودة بالفعل.")
            else:
                save_flight_data(f_id, {"flight_no": new_f, "reg": new_r, "date": today_str, "times": {}})
                st.session_state.active_id = f_id
                st.rerun()
else:
    if active_flights:
        st.session_state.active_id = st.sidebar.selectbox("اختر رحلة:", active_flights)
        if st.sidebar.button("🗑️ حذف هذه الرحلة"):
